...
use crate::encoding::v1::keys::index_v2::{GlobalIndexV2Key, GlobalIndexV2Kind, IndexV2Key};
...
use super::{
    ...
    IndexStateTransition, IndexStateV2, IndexV2MetadataValue, OperationClaim,
    OperationQueuePointerValue, WriterEpoch,
};
...
pub(crate) enum OperationPointerObservation {
    ...
    /// An exact text child still owns this queued operation revision.
    WaitingOnChild,
...
impl SameEpochRecoveryProof {
    ...
    /// Whether this proof authorizes replacement inside the supplied epoch.
    pub(super) fn authorizes(self, writer_epoch: WriterEpoch) -> bool {
...
///
/// Ordinary family work carries only its coordination permit. Text partition
/// construction instead carries a closed prepared upload value whose split and
/// coordinator reservation were created before the repository transaction.
/// Keeping this distinction in an enum prevents a text upload from being
/// mistaken for an ordinary step that may be discarded without releasing its
...
pub(crate) enum PreparedIndexOperationStep {
    ...
    ReaderLifecycle {
        family: IndexOperationFamily,
        _permit: Box<dyn IndexOperationStepPermit>,
        prepared: Box<super::reader_lifecycle::PreparedReaderLifecycleStep>,
    },
    Text(Box<super::text::driver::PreparedTextOperationStep>),
}

/// Exact outcome of resolving an operation commit error by point reads.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum PreparedStepCommitResolution {
    /// This step has no external reservation requiring commit resolution.
    Ordinary,
    /// The exact next operation and its prepared child are both durable.
    Committed,
    /// The exact claimed operation remains and the prepared child is absent.
    NotCommitted,
}
...
impl PreparedIndexOperationStep {
    ...
    /// Retains one external reader-lifecycle preparation through DB commit.
    pub(crate) fn reader_lifecycle(
        family: IndexOperationFamily,
        permit: Box<dyn IndexOperationStepPermit>,
        prepared: super::reader_lifecycle::PreparedReaderLifecycleStep,
    ) -> Self {
        Self::ReaderLifecycle {
            family,
            _permit: permit,
    ...
    pub(crate) fn family(&self) -> IndexOperationFamily {
        match self {
            Self::DriverOwned { family, .. } | Self::ReaderLifecycle { family, .. } => *family,
    ...
    async fn stage(
        ...
        match self {
            ...
            Self::ReaderLifecycle { prepared, .. } => match prepared.repository_result() {
                Some(result) => Ok(result),
                None => driver.step(db, transaction, scope, operation, limits).await,
            },
            Self::Text(step) => step.stage(transaction, scope, operation).await,
        }
    }
    ...
    async fn discard(self) -> Result<()> {
        match self {
            Self::DriverOwned { .. } | Self::ReaderLifecycle { .. } => Ok(()),
            Self::Text(step) => (*step).discard().await,
        }
    }

    /// Resolves an ambiguous commit without inferring from blob presence.
    async fn resolve_commit_error(
        &self,
        db: &Db,
        scope: DataScope,
    ) -> Result<PreparedStepCommitResolution> {
        match self {
            Self::DriverOwned { .. } | Self::ReaderLifecycle { .. } => {
                Ok(PreparedStepCommitResolution::Ordinary)
            }
            Self::Text(step) => step.resolve_commit_error(db, scope).await,
        }
    }
...
pub(crate) async fn observe_operation_pointer(
    ...
    if matches!(
        operation.execution_state(),
        IndexOperationExecutionState::Queued { .. }
    ) && super::text::driver::await_upload_child_is_pending(
        &transaction,
        pointer.scope,
        &operation,
    )
    .await?
    {
        return Ok(OperationPointerObservation::WaitingOnChild);
    }
    ...
    match operation.execution_state() {
        ...
        } if *not_before > now_unix_millis => Ok(OperationPointerObservation::Delayed {
    ...
    > = async {
        ...
        let committed = match step {
            ...
            IndexOperationStepResult::TransientFailure => {
                unreachable!("transient steps return before any physical transaction can commit")
            }
        };
        failpoints::trip(IndexOutboxFailpoint::CheckpointStagingAfter)?;
        failpoints::trip(IndexOutboxFailpoint::CommitBefore)?;
        Ok(Some((transaction, index, operation, committed)))
    }
    .await;

    let Some((transaction, index, operation, committed)) = (match staged {
        Ok(staged) => staged,
        Err(error) => {
            prepared.discard().await?;
            return Err(error);
        ...
        return Ok(CommittedOperationStep::TransientFailure);
    };

    match transaction.commit().await {
        Ok(_) => {}
        Err(commit) => match prepared.resolve_commit_error(db, claimed.scope).await? {
            PreparedStepCommitResolution::Ordinary => return Err(commit.into()),
...
/// Stages one exact text-operation block discovered by its child upload.
///
/// The caller stages the matching upload block in this same transaction. This
/// boundary owns operation encoding and pointer removal so text code never
/// reconstructs operation repository keys outside the canonical repository.
pub(super) fn stage_blob_mismatch_block(
    transaction: &DbTransaction,
    scope: DataScope,
    index: &IndexRecordV2,
    operation: &IndexOperationRecord,
    ...
    let next = operation
        .block_for_blob_mismatch(intent_id)
        .map_err(operation_model_error)?;
    validate_link(scope, index, &next, None)?;
    transaction.put(
        scoped_operation_key(scope, next.operation_id()),
        ...
        IndexOperationExecutionState::Blocked(blocker) => {
            ...
            match blocker {
                IndexOperationBlocker::BlobPublicationMismatch { intent_id } => {
                    super::text::upload_queue::stage_blob_mismatch_retry(
                        &transaction,
                        scope,
                        &operation,
                        &next,
