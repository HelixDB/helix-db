...
pub(crate) enum VectorMemoryAccess {
    ...
    ReadSnapshot {
        /// Exact-index resident store retained by the managed read lease.
...
impl VectorMemoryStore {
        ...
        R: DbReadOps + Send + Sync,
    {
        ...
        loop {
            ...
            let candidate_total = estimated_bytes.checked_add(row_bytes).ok_or_else(|| {
...
fn estimated_entry_bytes(key_len: usize, value_len: usize) -> Result<u64, HelixDbError> {
    let key_len = u64::try_from(key_len).map_err(|_| {
        HelixDbError::InvariantViolation("vector memory key length exceeds u64".to_string())
    })?;
    let value_len = u64::try_from(value_len).map_err(|_| {
        HelixDbError::InvariantViolation("vector memory value length exceeds u64".to_string())
    })?;
    key_len
