...
pub(crate) async fn hydrate_active_generations(
    ...
    for (ordinal, handle) in targets.into_iter().enumerate() {
        ...
        admitted_bytes = admitted_bytes
            .checked_add(summary.estimated_bytes)
            .ok_or_else(|| {
                HelixDbError::InvariantViolation(
        ...
        match hydration {
            ...
            VectorCacheHydration::Unavailable(_) => {
                unreachable!("unavailable hydration was handled before storage I/O")
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    async fn active_hydration_publishes_exact_snapshot_and_refreshes_immutably() {
        ...
        .await
        .unwrap();
        let first_lease = registry.lease_for(&handle).unwrap();
        assert_eq!(first_lease.store().visible_seq(), first_snapshot.seq());
        assert_eq!(
            first_lease.store().get_upper_vector(1).as_deref(),
        ...
        .await
        .unwrap();
        let second_lease = registry.lease_for(&handle).unwrap();
        assert_eq!(second_lease.store().visible_seq(), second_snapshot.seq());
        assert!(second_lease.store().get_upper_vector(2).is_some());
        assert!(first_lease.store().get_upper_vector(2).is_none());
        ...
        .await
        .unwrap();
        let empty_lease = registry.lease_for(&handle).unwrap();
        assert_eq!(empty_lease.store().estimated_bytes(), 0);
        assert!(empty_lease.store().get_upper_vector(1).is_none());
        assert!(empty_lease.store().get_upper_vector(2).is_none());
        assert!(second_lease.store().get_upper_vector(2).is_some());
    }
    ...
    async fn partitioned_hydration_requires_a_cross_checked_v2_mapping() {
        ...
        .await
        .unwrap();
        let lease = registry.lease_for(&handle).unwrap();
        assert_eq!(
            lease.store().get_upper_vector(3).as_deref(),
    ...
    async fn the_same_physical_id_is_valid_in_distinct_scopes() {
        ...
        let first = registry.lease_for(&first_handle).unwrap();
        let second = registry.lease_for(&second_handle).unwrap();
        assert_eq!(
    ...
    async fn duplicate_physical_ids_in_one_scope_fail_before_publication() {
        ...
        assert!(registry.lease_for(&first_handle).is_err());
        assert!(registry.lease_for(&second_handle).is_err());
    }
    ...
    async fn mismatched_partition_mapping_fails_before_publication() {
        ...
        assert!(matches!(error, HelixDbError::IndexCatalogCorruption(_)));
        assert!(registry.lease_for(&handle).is_err());
    }
