...
use super::mutation::{MutationOpCache, VectorInsertContract};
#[cfg(test)]
...
use super::storage::{
    CanonicalVectorRowKey, VectorCleanupRow, VectorCleanupScan, VectorRowKeyspace, VectorRows,
    VectorWriteRows,
};
use super::{
...
#[cfg(test)]
use super::{encode_item, encode_metadata, index_id_from_name, SimHashMode};
...
const OP_LOCAL_ITEM_CACHE_LIMIT: usize = 4_096;
...
impl<D: Distance> VectorIndex<D> {
    ...
    /// transaction, admits the returned complete measurement, and then replays
    /// the same layer in its commit transaction. The physical generation must
    /// remain exclusively builder-owned between those transactions. Supplying
    /// the layer explicitly prevents random layer selection from changing the
    /// measured graph invariant during replay. Planning must use a write
    /// cache-disabled index handle so an aborted planning transaction cannot
    /// affect a resident runtime snapshot.
    #[cfg(test)]
    ...
    /// captures a write checkpoint before this call, and then reads both
    /// `measurement_since(checkpoint)` and cumulative `measurement()`. The
    /// The closed memory-access ADT has no resident-store mutation mode, so
    ...
    /// replays the same delete-plus-insert graph mutation at commit. Existing
    /// vector bytes and HNSW encodings are unchanged; the explicit layer makes
    /// the measured and replayed write sets comparable.
    ...
    pub(super) async fn get_item_for_layer_cached(
        ...
        if let Some(cached) = mutation_cache.items.get(&key) {
            return Ok(cached.clone());
        }

        if mutation_cache.items.len() >= OP_LOCAL_ITEM_CACHE_LIMIT {
            mutation_cache.items.clear();
        }
        ...
        let loaded = self
            ...
            .await?
            .map(Arc::new);
        mutation_cache.items.insert(key, loaded.clone());
        Ok(loaded)
    }
    ...
    pub(super) async fn get_items_for_layer_cached_batch(
        ...
        if mutation_cache.items.len().saturating_add(node_ids.len()) >= OP_LOCAL_ITEM_CACHE_LIMIT {
            mutation_cache.items.clear();
        }
        ...
        for &node_id in node_ids {
            ...
            if let Some(cached) = mutation_cache.items.get(&key) {
                if let Some(item) = cached {
                    result.insert(node_id, item.clone());
                }
                continue;
            }
        ...
        if layer > 0 {
            ...
            for (node_id, maybe_row) in missing.into_iter().zip(upper_rows) {
                ...
                mutation_cache
                    .items
                    .insert((layer, node_id), Some(item.clone()));
        ...
        let (canonical_keys, _) = self
            .resolve_canonical_vector_keys_batch_counted::<true>(
                txn,
                &missing,
        ...
        if !missing_simhash_ids.is_empty() {
            ...
            for (node_id, row_exists) in missing_simhash_ids.into_iter().zip(layer0_rows_exist) {
                ...
                mutation_cache.items.insert((layer, node_id), None);
            }
        }
...
mod tests {
    ...
    async fn test_phase0_public_result_and_io_baseline() {
        ...
        assert_eq!(stats.object_store_reads, 12);
    ...
    async fn test_known_fresh_insert_adds_new_vector() {
        ...
        let index = create_test_vector_index(&db, "known_fresh_insert_adds_new_vector_idx").await;
        ...
        for node_id in node_ids {
            ...
            match single {
                ...
                None => {
                    ...
                    assert!(
                        matches!(batch_cache.items.get(&(0, node_id)), Some(None)),
        ...
        for node_id in node_ids {
            ...
            match single {
                ...
                None => {
                    ...
                    assert!(
                        matches!(batch_cache.items.get(&(2, node_id)), Some(None)),
