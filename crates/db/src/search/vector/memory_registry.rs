...
//! must hold a [`VectorCacheLease`] while using its store; retirement
//! changes the entry to `Retiring`, rejects new leases, waits for existing
//! leases and hydration to finish, acquires the independent all-dirty
...
pub(crate) enum VectorCacheLifecycle {
    ...
    Hydrating,
    /// A fully published store may issue active-reader leases.
    Ready,
    /// New leases are rejected while hydration/readers/publication drain.
    Retiring,
    /// The store is cleared and may no longer issue leases or be hydrated.
    Closed,
}
...
impl VectorMemoryCacheEntry {
    ...
    /// Returns the complete identity that every lease from this entry retains.
    #[cfg(any(test, feature = "production-coverage"))]
    ...
    ///
    /// The lease retains both the exact identity and immutable store `Arc`.
    ...
    pub(crate) fn acquire_lease(
        self: &Arc<Self>,
    ) -> Result<VectorCacheLease, VectorCacheLeaseError> {
        let store = {
            let mut state = self.state.lock();
            let VectorCacheEntryState::Ready(resident) = &mut *state else {
                return Err(VectorCacheLeaseError::Unavailable(state.lifecycle()));
            };
    ...
    ///
    /// The first call atomically rejects new leases. It then waits for any
    /// unpublished hydration and active leases, independently acquires the
            ...
            let drained = {
                ...
                match &*state {
                    ...
                    VectorCacheEntryState::Hydrating | VectorCacheEntryState::Ready(_) => {
                        unreachable!("retirement cannot return to a lease-admitting state")
                    }
                }
            };
            if drained {
...
impl VectorCacheRefresh {
    ...
    ///
    /// Existing leases keep their previous `Arc`. Equal visibility may replace
...
pub(crate) struct VectorCacheLease {
    entry: Arc<VectorMemoryCacheEntry>,
    store: Arc<VectorMemoryStore>,
}

impl VectorCacheLease {
    /// Returns the exact identity whose active-reader count this lease owns.
...
enum VectorCacheCommitEvidence {
    ...
    Sequence(u64),
    /// An independent atomic commit proof established success after SlateDB
    /// returned an error, so no write handle or sequence is available.
    ExactProof,
}
...
impl VectorCachePendingCommit {
    ...
    /// Evicts rows after an atomic proof establishes a committed transaction.
    ///
    /// SlateDB does not return a [`slatedb::WriteHandle`] when `commit` reports
    /// an error, even if an exact proof in that transaction later establishes
    /// that its writes committed. Eviction remains mandatory, but the usual
    /// visible-sequence assertion cannot be made without inventing a sequence.
    pub(crate) async fn evict_after_proven_commit_without_sequence(self) {
        self.evict(VectorCacheCommitEvidence::ExactProof).await;
    }
    ...
    async fn evict(self, evidence: VectorCacheCommitEvidence) {
        let _publication = self.entry.pending_dirty.lock_publish().await;
        {
            ...
            match evidence {
                ...
                VectorCacheCommitEvidence::ExactProof => {}
            }
...
impl Drop for VectorCacheLease {
    fn drop(&mut self) {
        {
            let mut state = self.entry.state.lock();
            let resident = match &mut *state {
                ...
                | VectorCacheEntryState::Retiring { resident: None, .. }
                | VectorCacheEntryState::Closed => {
                    unreachable!("a live vector cache lease must retain resident state")
                }
            };
            resident.active_readers = resident
                .active_readers
                .checked_sub(1)
                .expect("a vector cache lease releases one acquired reader");
        }
        self.entry.changed.notify_waiters();
    }
}

/// Reason an exact vector cache entry cannot issue a reader lease.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub(crate) enum VectorCacheLeaseError {
...
impl VectorCacheRegistry {
    ...
    pub(crate) fn prepare_commit(
        ...
        if write.dirty_rows().is_empty() {
            return None;
        }
        let identity = VectorCacheIdentity::from_validated(write.handle());
        let entry = self.state.read().entries.get(&identity).cloned()?;
        let pending_guard = entry.pending_dirty.acquire(write.dirty_rows());
        Some(VectorCachePendingCommit {
            entry,
            dirty_rows: Arc::clone(write.dirty_rows()),
            _pending_guard: pending_guard,
        })
    }

    /// Acquires a reader lease only for an already admitted exact generation.
    ///
    ...
    pub(crate) fn lease_for(
        ...
        let Some(entry) = self.state.read().entries.get(&identity).cloned() else {
            return Err(VectorCacheLeaseError::Absent);
        };
        entry.acquire_lease()
    }
    ...
    ///
    /// Drop calls this after durable reader leases are drained and before the
    /// first physical delete. If no cache was admitted, it still installs a
    /// closed tombstone so hydration cannot race the later physical cleanup.
    #[cfg(any(test, feature = "production-coverage"))]
...
mod tests {
    ...
    #[tokio::test]
    async fn retirement_rejects_new_leases_and_waits_for_active_reader() {
    ...
    async fn pending_commit_fences_then_evicts_while_abort_publishes_nothing() {
        ...
        store.insert_simhash(7, crate::search::vector::SimHash::from_bits(12));
        store.insert_upper_vector(7, Bytes::from_static(b"proof-committed"));
        let proof_committed = registry.prepare_commit(&write).unwrap();
        proof_committed
            .evict_after_proven_commit_without_sequence()
            .await;
        assert!(store.get_simhash(7).is_none());
        assert!(store.get_upper_vector(7).is_none());
    ...
    async fn commit_generation_changes_discard_initial_and_refresh_hydration() {
        ...
        assert!(Arc::ptr_eq(
            registry.lease_for(&handle).unwrap().store(),
            &resident
        ));
    }
