...
impl<'db> ExecutionContext<'db> {
    ...
    async fn drop_edges_between(
        ...
        // Observe every requested pair directly. Adjacency is an optimization
        // hint, not authority: using it to select pairs could leave a valid
        // pair row undeleted when an adjacency row is missing or corrupt.
        let pairs = source_nodes
            .iter()
            .flat_map(|from| targets.iter().map(move |target| (*from, *target)))
            .collect::<BTreeSet<_>>();
