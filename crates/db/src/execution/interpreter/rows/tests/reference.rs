    ...
    for seed in 0..4 {
        ...
        let mut cases = vec![
            ...
            Case { query: "MATCH (a:N)-[r:R]->(b:N) WITH a.key AS a,count(*) AS c RETURN a,c ORDER BY a,c",
                columns: &["a", "c"], rows: degrees.clone() },
            Case { query: "MATCH (a:N)-[r:R]->(b:N) RETURN a.key AS a,1+count(*) AS c ORDER BY a,count(*)",
                columns: &["a", "c"], rows: degrees.iter().map(|row| vec![row[0],row[1].map(|count| count+1)]).collect() },
            Case { query: "MATCH (a:N)-[r:R]->(b:N) RETURN 1+count(*) AS c ORDER BY 2-count(*)",
                columns: &["c"], rows: vec![vec![Some(1+directed.len() as i64)]] },
            Case { query: "MATCH (a:N)-[r:R]->(b:N) RETURN 1+sum(b.key)+count(*) AS total",
                columns: &["total"], rows: vec![vec![Some(1+directed.iter().map(|(_,_,b)| b).sum::<i64>()+directed.len() as i64)]] },
