...
| `scoped_search_gate` | `test` | `crates/db/tests/scoped_search_gate.rs` | Requires `production-scale`; one ignored release-only gate on the synthetic `Group <- Item -> Attribute` fixture shared with the `scoped_search_bench` example. Every traversal-scoped vector shape must return only in-scope IDs with mean recall@k of at least 0.95 against a client-side exact scan, and a post-expansion stored-property filter must count exactly the rows a client-side filter keeps. Like the other `production-scale` targets, it is excluded from production-only coverage. |
