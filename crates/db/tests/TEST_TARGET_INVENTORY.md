...
| `production_migration_contracts` | `test` | `crates/db/tests/production_migration_contracts.rs` | Requires `production-coverage`, `migration-parity`, and `index-lifecycle-testing`; covers legacy definition convergence, populated-vector adoption, malformed and ineligible vector lanes, ownership conflicts, invalid V2 bootstrap tuples, failure-preserving resume, and active/conflicting definition handling through production migration boundaries. |
...
| `production_text_correctness_regressions` | `test` | `crates/db/tests/production_text_correctness_regressions.rs` | Requires `production-coverage`, `migration-parity`, and `index-lifecycle-testing`; runs bounded text correctness regressions through the compiled production library. |
...
The shared text oracle and fixture helpers live in
`crates/db/tests/production_support/text_correctness.rs`. They have no standalone
test entry points and are exercised by `production_migration_contracts` and
`production_text_correctness_regressions`; Cargo must not discover an empty
helper executable as an independent test target.
