...
This project authors HelixDB queries with the **TypeScript DSL** (`@helix-db/helix-db`) — typed builder functions, not hand-written JSON. Never write Rust `.hx` files; there is no compile step. Queries live as builder modules under `web/src/lib/queries/*.ts`; a small server-only runner (`web/src/lib/helix.ts`) serializes them and POSTs to the local DB at `http://localhost:6969/v1/query`. Raw query JSON (`helix query dev --file/--json`) stays available as a fallback for debugging. The frontend that consumes these queries is a Next.js app under `web/` (App Router, TypeScript, Tailwind) — see `<frontend>`.
</environment>
