...
pub(super) fn rewrite_access_order_range_direction(
    ...
    if let ir::NodeAccessPlan::Intersect(children) = source.as_ref()
        && source.is_secondary_set_eligible()
    {
        ...
        while let Some(child) = pending.pop() {
            match child.as_ref() {
                ir::NodeAccessPlan::Intersect(nested) => pending.extend(nested.iter().rev()),
                _ => leaves.push(child.clone()),
            }
        }
        let selected = leaves.iter().enumerate().find_map(|(position, child)| {
            if let ir::NodeAccessPlan::RangeIndex { key, .. } = child.as_ref()
