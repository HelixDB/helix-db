...
QUERY testRRFDefault(query_vec: [F32]) =>
...
QUERY testMMRDefault(query_vec: [F32]) =>
    results <- SearchV<Document>(query_vec, 100)
...
QUERY testMMREuclidean(query_vec: [F32]) =>
    results <- SearchV<Document>(query_vec, 100)
...
QUERY testMMRDotProduct(query_vec: [F32]) =>
    results <- SearchV<Document>(query_vec, 100)
...
QUERY testChainedRerankers(query_vec: [F32]) =>
    results <- SearchV<Document>(query_vec, 100)
        ::RerankRRF(k: 60)
        ::RerankMMR(lambda: 0.7)
        ::RANGE(0, 10)
...
QUERY testMultipleMMR(query_vec: [F32]) =>
    results <- SearchV<Document>(query_vec, 100)
