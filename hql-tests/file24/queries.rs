...
.check_property("name").collect_to::<Vec<_>>())?;
value_remapping!(remapping_vals, item.clone(), "followerCount" => G::new_from(Arc::clone(&db), &txn, vec![item.clone()])
...
.add_n("Post", Some(props! { "content" => data.content.clone(), "updated_at" => data.now.clone(), "created_at" => data.now.clone() }), None).collect_to::<Vec<_>>();
    G::new_mut(Arc::clone(&db), &mut txn)
