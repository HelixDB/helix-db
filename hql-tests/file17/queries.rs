...
let mut txn = db.graph_env.write_txn().unwrap();
    let updatedUsers = {let update_tr = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.userID)
    .collect_to::<Vec<_>>();G::new_mut_from(Arc::clone(&db), &mut txn, update_tr)
    .update(Some(props! { "name" => data.name, "age" => data.age }))
    .collect_to::<Vec<_>>()};
let mut return_vals: HashMap<String, ReturnValue> = HashMap::new();
        return_vals.insert("updatedUsers".to_string(), ReturnValue::from_traversal_value_array_with_mixin(updatedUsers.clone(), remapping_vals.borrow_mut()));
