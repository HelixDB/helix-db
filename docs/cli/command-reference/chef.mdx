...
title: "helix chef"
sidebarTitle: "chef"
description: "Bootstrap a first HelixDB app with a coding agent"
pageType: "Reference"
---

<div className="flex flex-wrap gap-2"><Badge color="gray" size="sm">Reference</Badge></div>

Bootstrap a first HelixDB app for a coding agent. `helix chef` installs agent context, scaffolds a local project, starts the local database, seeds starter data, writes a build prompt, and launches your coding agent to generate the app. Local only.
...
- `helix chef` is a local workflow and does not require Cloud authentication. It neither uploads a project snapshot nor reads Cloud credentials. Use `helix auth login` separately only when you later run a Cloud command.
- Without a TTY, setup runs with defaults and no agent is launched; point your own agent at `HELIX_CHEF_PROMPT.md`.
...
Before launching the agent, the CLI asks how much autonomy to give it: full autonomy, ask before each shell command, or don't launch (print the prompt path instead).
...
- [`helix skills`](/cli/command-reference/skills) — manage the agent skills that `chef` installs.
