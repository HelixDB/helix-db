...
| `--disk` | Boolean | Use on-disk storage backed by a CLI-managed SeaweedFS container for this run. | `false` |
| `--image-version` | String | Override the image tag or `sha256:<64 lowercase hex digits>` for this run. | Configured tag |
| `--pull` | String | `always`, `missing`, or `never`. Explicit policies also apply to the disk-mode SeaweedFS image. | Configured policy, otherwise `always` for `latest`, `missing` for other versions |
...
- Default storage is in-memory. Passing `--disk` starts a SeaweedFS S3 sidecar (`ghcr.io/chrislusf/seaweedfs:4.47`, pinned by digest) that creates the `helix-db` bucket, waits until the bucket accepts signed requests, and runs `helixdb` with S3-compatible storage environment variables.
...
With `--disk`, `helix stop` removes the Helix and SeaweedFS containers but keeps the persistent local volume. `helix prune` removes that volume and deletes the persisted local data.
