...
export const movieGraphDumpV1Schema = z
  .object({
    ...
    nodes: z.array(
      z.discriminatedUnion("kind", [personNodeSchema, movieNodeSchema]),
    ),
...
export type MovieGraphRelationship =
...
export function parseMovieGraphDump(input: unknown): MovieGraphDumpV1 {
  ...
  if (duplicatePerson !== undefined)
    throw new TypeError(`duplicate Person.personId: ${duplicatePerson}`);
  const duplicateMovie = duplicate(movies.map((movie) => movie.movieId));
  if (duplicateMovie !== undefined)
    throw new TypeError(`duplicate Movie.movieId: ${duplicateMovie}`);
  const duplicateRelationship = duplicate(
    dump.relationships.map((relationship) => relationship.relationshipId),
  );
  if (duplicateRelationship !== undefined)
    throw new TypeError(`duplicate relationshipId: ${duplicateRelationship}`);
  ...
  for (const relationship of dump.relationships) {
    if (!personIds.has(relationship.fromPersonId)) {
      throw new TypeError(
        `${relationship.relationshipId} references missing Person ${relationship.fromPersonId}`,
      );
    }
    if (!movieIds.has(relationship.toMovieId)) {
      throw new TypeError(
        `${relationship.relationshipId} references missing Movie ${relationship.toMovieId}`,
      );
    }
    if (relationship.kind === "acted_in") {
      ...
      if (duplicateRole !== undefined)
        throw new TypeError(
          `${relationship.relationshipId} contains duplicate role ${duplicateRole}`,
        );
    }
  }
