...
import {
  parseMovieGraphDump,
  type MovieGraphDumpV1,
  type MovieGraphNode,
  type MovieGraphRelationship,
} from "./model.js";
...
export async function readDump(path: string): Promise<MovieGraphDumpV1> {
  ...
  if (match === null || match[2] !== basename(path))
    throw new TypeError(`invalid checksum file: ${path}.sha256`);
...
export function chunks<T>(items: readonly T[], size: number): T[][] {
  if (!Number.isSafeInteger(size) || size <= 0)
    throw new TypeError(`batch size must be a positive integer: ${size}`);
