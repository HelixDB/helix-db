...
const rowsParams = defineParams({
  rows: param.array(param.object(param.value())),
});
  ...
  return writeBatch()
    .varAs(
      ...
      g().createIndexIfNotExists(
        IndexSpec.nodeUniqueEquality("Person", "personId"),
      ),
    )
    .varAs(
      "movie_id",
      g().createIndexIfNotExists(
        IndexSpec.nodeUniqueEquality("Movie", "movieId"),
      ),
    )
    .varAs(
      "acted_in_id",
      g().createIndexIfNotExists(
        IndexSpec.edgeEquality("ACTED_IN", "relationshipId"),
      ),
    )
    .varAs(
      "directed_id",
      g().createIndexIfNotExists(
        IndexSpec.edgeEquality("DIRECTED", "relationshipId"),
      ),
    )
    .varAs(
      "produced_id",
      g().createIndexIfNotExists(
        IndexSpec.edgeEquality("PRODUCED", "relationshipId"),
      ),
    )
    .returning([
      "person_id",
...
export function upsertPeopleRequest(
  ...
  const rows = people.map(({ personId, name, born }) => ({
    personId,
    name,
    born,
  }));
...
export function upsertMoviesRequest(
  movies: readonly MovieNode[],
): QueryRequest {
  const body = writeBatch()
    .varAs(
      "existing",
      g().nWithLabel("Movie").where(Predicate.eqParam("movieId", "movieId")),
    )
    .varAsIf(
      "updated",
      BatchCondition.varNotEmpty("existing"),
  ...
  const rows = movies.map(({ movieId, title, released, tagline }) => ({
    movieId,
    title,
    released,
    tagline,
  ...
  return writeBatch()
    .forEachParam("rows", body)
    .returning(["updated", "created"])
    .toQueryRequest(rowsParams, { rows }, { queryName: "upsert_movies" });
}

type Relationship =
  ActedInRelationship | DirectedRelationship | ProducedRelationship;

export function replaceRelationshipsRequest(
  ...
  const label =
    kind === "acted_in"
      ...
      : kind === "directed"
        ? "DIRECTED"
        : "PRODUCED";
  const properties: Record<string, PropertyInput> = {
    relationshipId: PropertyInput.param("relationshipId"),
  };
  ...
  const body = writeBatch()
    .varAs(
      ...
      g()
        .nWithLabel("Person")
        .where(Predicate.eqParam("personId", "fromPersonId")),
    )
    .varAs(
      "to",
      g().nWithLabel("Movie").where(Predicate.eqParam("movieId", "toMovieId")),
    )
    .varAs(
      "existing",
      g()
        .eWithLabel(label)
        .where(Predicate.eqParam("relationshipId", "relationshipId")),
    )
    .varAsIf(
      "removed",
      BatchCondition.varNotEmpty("existing"),
      g().dropEdgeById(EdgeRef.var("existing")),
    )
    .varAs(
      "created",
      g().n(NodeRef.var("from")).addE(label, NodeRef.var("to"), properties),
    );
  ...
  return writeBatch()
    ...
    .toQueryRequest(
      rowsParams,
      { rows },
      { queryName: `replace_${kind}_relationships` },
    );
}
