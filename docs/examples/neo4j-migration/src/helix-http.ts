...
export class HelixHttpClient {
  ...
  async execute(request: QueryRequest, options: HelixRequestOptions = {}): Promise<unknown> {
    const attempts = options.attempts ?? 8;
    const baseDelayMs = options.baseDelayMs ?? 10;
    if (!Number.isSafeInteger(attempts) || attempts <= 0) throw new TypeError(`attempts must be a positive integer: ${attempts}`);
    if (!Number.isSafeInteger(baseDelayMs) || baseDelayMs < 0) throw new TypeError(`baseDelayMs must be a non-negative integer: ${baseDelayMs}`);

    const headers: Record<string, string> = { "content-type": "application/json" };
    if (options.awaitDurability === true) headers["x-helix-await-durable"] = "true";
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      ...
      if (response.ok) return responseBody.length === 0 ? undefined : (JSON.parse(responseBody) as unknown);
      if (response.status === 409 && attempt + 1 < attempts) {
        await this.sleep(baseDelayMs * 2 ** attempt);
        continue;
      }
      throw new HelixHttpError(response.status, responseBody || response.statusText);
    }
    throw new Error("HelixDB conflict retry loop exhausted unexpectedly");
  }
}

export async function waitForHelix(url: string, attempts = 120, fetchImpl: FetchLike = fetch): Promise<void> {
