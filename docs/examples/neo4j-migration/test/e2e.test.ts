...
import { expectedCounts, helixCounts, verifyMigration } from "../src/verification.js";
...
async function compose(...args: string[]): Promise<void> {
  await execute("docker", ["compose", ...args], { cwd: exampleRoot, timeout: 180_000 });
}

test("migrates Neo4j data, resumes after interruption, and replays without duplicates", { timeout: 300_000 }, async () => {
  ...
  try {
    ...
    const dump = await exportNeo4jSnapshot(driver, config.neo4jDatabase, "2026-08-11T00:00:00.000Z");
    ...
    await loadDump(validatedDump, client, { batchSize: 2, stopAfterNodes: true });
    assert.deepEqual(await helixCounts(client), { ...expectedCounts(validatedDump), acted_in: 0, directed: 0, produced: 0 });

    await loadDump(validatedDump, client, { batchSize: 2 });
    await verifyMigration(driver, config.neo4jDatabase, client, validatedDump);
