...
const countRequest = QueryRequest.read(
  readBatch()
    .varAs("count", g().nWithLabel("Movie").count())
    .returning(["count"]),
);
...
test("retries 409 responses with bounded exponential backoff", async () => {
  const responses = [
    ...
    new Response("conflict", { status: 409 }),
    new Response('{"count":2}', { status: 200 }),
  ];
  const delays: number[] = [];
  ...
  assert.deepEqual(
    await client.execute(countRequest, {
      attempts: 3,
      baseDelayMs: 5,
  ...
  assert.equal(
    (calls[0]!.headers as Record<string, string>)["x-helix-await-durable"],
    "true",
  );
...
test("does not retry permanent errors or an exhausted conflict", async () => {
  const permanent = new HelixHttpClient(
    "http://localhost:16969",
    async () => new Response("bad query", { status: 400 }),
  );
  await assert.rejects(
    permanent.execute(countRequest),
    (error) => error instanceof HelixHttpError && error.status === 400,
  );
  const conflict = new HelixHttpClient(
    "http://localhost:16969",
    async () => new Response("conflict", { status: 409 }),
    async () => {},
  );
  await assert.rejects(
    conflict.execute(countRequest, { attempts: 2 }),
    (error) => error instanceof HelixHttpError && error.status === 409,
  );
  await assert.rejects(
    conflict.execute(countRequest, { attempts: 0 }),
    /positive integer/,
  );
  await assert.rejects(
    conflict.execute(countRequest, { baseDelayMs: -1 }),
    /non-negative integer/,
  );
});

test("returns undefined for an empty successful response and rejects malformed JSON", async () => {
  const empty = new HelixHttpClient(
    "http://localhost:16969",
    async () => new Response(null, { status: 200 }),
  );
  assert.equal(await empty.execute(countRequest), undefined);
  const malformed = new HelixHttpClient(
    "http://localhost:16969",
...
test("serializes both migration reads as parameterized direct requests", () => {
  const movie = JSON.parse(
    movieDetailsRequest("m-matrix").toJsonString(),
  ...
  const filmography = JSON.parse(
    personFilmographyRequest("p-keanu").toJsonString(),
  ) as Record<string, unknown>;
...
test("keeps the guide's raw JSON tabs equal to the TypeScript serializer", async () => {
  const guide = await readFile(
    new URL(
      "../../../database/helix-db/migration-guides/from-neo4j.mdx",
      import.meta.url,
    ),
    "utf8",
  );
  const requests = [...guide.matchAll(/```json JSON\n([\s\S]*?)\n```/g)]
    .map((match) => JSON.parse(match[1]!) as Record<string, unknown>)
    .filter((value) => typeof value.query_name === "string");
  assert.deepEqual(requests, [
    JSON.parse(movieDetailsRequest("m-matrix").toJsonString()),
    JSON.parse(personFilmographyRequest("p-keanu").toJsonString()),
  ]);
});
...
test("serializes replay-safe writes, indexes, and count verification", () => {
  ...
  for (const request of requests)
    assert.doesNotThrow(() => JSON.parse(request.toJsonString()));
