...
function clientWith(
  handler: (request: QueryRequest) => unknown | Promise<unknown>,
): HelixHttpClient {
  return {
...
test("stops cleanly after nodes and resumes with bounded relationship batches", async () => {
  ...
  const stopped = await loadDump(dump, client, {
    batchSize: 3,
    stopAfterNodes: true,
  });
...
test("waits for accepted indexes and rejects blocked or timed-out operations", async () => {
  ...
  await ensureIndexes(
    clientWith((request) => {
      const name = (
        JSON.parse(request.toJsonString()) as { query_name: string }
      ).query_name;
  ...
  await assert.rejects(
    ensureIndexes(
      clientWith((request) =>
        (JSON.parse(request.toJsonString()) as { query_name: string })
          .query_name === "bootstrap_movie_graph_indexes"
          ? { index: receipt }
...
test("normalizes Neo4j objects and Helix result arrays to one contract", () => {
  ...
  assert.deepEqual(
    normalizePersonFilmography({ person: [person], movies: [older, newer] }),
    { person, movies: [newer, older] },
  );
  assert.equal(normalizePersonFilmography({ person: [] }), null);
});

test("derives expected counts from the validated dump", async () => {
  assert.deepEqual(expectedCounts(await fixture()), {
    people: 5,
    movies: 2,
    acted_in: 6,
    directed: 2,
