...
Look up the stable `error` code in the [query error reference](/database/helix-db/query-guides/error-handling),
or in [Helix Cloud gateway errors](/database/helix-cloud/operate/error-handling) for
codes returned by a Cloud gateway.
...
## Request returns `rate_limit_unavailable` (Helix Cloud)
