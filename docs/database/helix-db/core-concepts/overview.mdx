...
description: "Build a small write query operation by operation, then read its request and response"
...
This walkthrough builds one query that creates two users, connects them with a
relationship, and reads that relationship back, all in a single transaction. Each step
shows what an operation contributes, what it becomes on the wire, and what the server
sends back. To run it, start a local instance as described in
