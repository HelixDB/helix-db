...
| `HELIX_DISK_CACHE_BYTES` | Total disk budget in bytes, at least 64 MiB; defaults to 32 GiB |
...
and the rest caches full-text search data. Startup fails with a message naming
the variable when a size is invalid, a size is set without `HELIX_DISK_CACHE_DIR`, or
