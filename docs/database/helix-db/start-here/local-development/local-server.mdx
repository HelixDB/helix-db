...
By default the server caches data in memory only, so cold reads go to the object
store. Set `HELIX_DISK_CACHE_DIR` to keep recently read data on local disk, ideally NVMe.
The cache survives restarts. It requires `S3_BUCKET` or `HELIX_DATA_DIR`; memory
storage rejects it.

<Note>
The disk cache needs an image released after v0.0.6. The v0.0.6 image and earlier
ignore the `HELIX_DISK_CACHE_*` variables and keep memory-only caches, so replace
`<version>` below with a later release.
</Note>
...
docker run --rm --name helixdb \
  ...
  ghcr.io/helixdb/helixdb:<version>
```

In Compose, add the same variables and a volume to the `helix` service. A new
named volume mounted at `/var/cache/helix` is already writable; a bind mount must be
owned by `65532:65532`. Use one cache directory per running server and per
database; changing `DB_PATH` on the same directory leaves the old database's
full-text cache behind.

| Variable | Purpose |
| --- | --- |
| `HELIX_DISK_CACHE_DIR` | Enables the disk cache in this directory, creating it if needed; unset keeps memory-only caches |
| `HELIX_DISK_CACHE_BYTES` | Total disk budget in bytes, from 64 MiB to 1 TiB; defaults to 32 GiB |
| `HELIX_DISK_CACHE_MEMORY_BYTES` | Memory tier of the block cache in bytes; defaults to 640 MiB |

Half of the disk budget caches object-store files, 3/8 caches database blocks,
and the rest caches full-text search data. Changing the budget usually changes the
block size, which discards the database-block cache at startup so it refills from
the object store. The other two caches keep their files and evict down to a smaller
budget.
...
Lowering the budget is not a reliable fix. Run natively on macOS, the limit is also
capped by `sysctl kern.maxfilesperproc`.

Startup also fails with a message naming the variable when a size is invalid or
out of range, a size is set without `HELIX_DISK_CACHE_DIR`, or the cache directory
is not writable.
