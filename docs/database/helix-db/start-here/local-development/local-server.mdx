...
owned by `65532:65532`. Use one cache directory per running server.
...
and the rest caches full-text search data. A smaller budget on restart trims the
existing cache.

The block cache keeps up to 32,768 files open, so the server needs up to 34,792
open files (26,600 at the default budget). It raises its soft open-file limit at
startup. If the container's hard limit is lower, startup fails naming
