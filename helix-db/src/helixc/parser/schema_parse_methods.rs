...
use crate::helixc::parser::{
    ...
    location::HasLoc,
    types::{
        DefaultValue, EdgeSchema, Field, FieldPrefix, FieldType, NodeSchema, Source, VectorSchema,
    },
...
impl HelixParser {
    pub(super) fn parse_node_def(
        ...
        let fields = self.parse_node_body(pairs.try_next()?, filepath)?;
        Ok(NodeSchema {
            name: (loc.clone(), name),
            fields,
            loc,
    ...
    pub(super) fn parse_vector_def(
        ...
        let name = pairs.try_next()?.as_str().to_string();
        let fields = self.parse_node_body(pairs.try_next()?, filepath)?;
        Ok(VectorSchema { name, fields, loc })
    }
    ...
    pub(super) fn parse_node_body(
        ...
        field_defs
            .into_inner()
            .map(|p| self.parse_field_def(p, filepath))
    ...
    pub(super) fn parse_edge_def(
        ...
        let edge_loc = pair.loc_with_filepath(filepath);
        let mut pairs = pair.into_inner();
        ...
        let properties = match body_pairs.next() {
            Some(pair) => Some(self.parse_properties(pair, filepath)?),
            None => None,
        };
