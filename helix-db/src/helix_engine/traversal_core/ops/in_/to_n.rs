...
#[cfg(feature = "slate")]
pub trait AsyncToNAdapter<'db, 'arena, 'txn>: Sized {
    fn to_n(
        self,
...
#[cfg(feature = "slate")]
impl<'db, 'arena, 'txn, S> AsyncToNAdapter<'db, 'arena, 'txn>
        ...
        let stream = async_stream::try_stream! {
            ...
            while let Some(item) = inner.next().await {
                ...
                if let TraversalValue::Edge(edge) = item {
                    let node = self.storage.get_node(self.txn, edge.to_node, self.arena).await?;
