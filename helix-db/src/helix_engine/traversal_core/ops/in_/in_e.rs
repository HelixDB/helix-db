            ...
            .filter_map(move |item| {
                ...
                match item {
                    Ok(item) => {
                        ...
                        let mut iter = self
                            .txn
                            .raw_prefix_iter(&self.storage.cf_in_edges(), &prefix);
...
#[cfg(feature = "slate")]
pub trait AsyncInEdgesAdapter<'db, 'arena, 'txn, 's>: Sized {
    fn in_e(
        self,
        edge_label: &'s str,
...
#[cfg(feature = "slate")]
impl<'db, 'arena, 'txn, 's, S> AsyncInEdgesAdapter<'db, 'arena, 'txn, 's>
