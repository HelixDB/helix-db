...
#[cfg(feature = "slate")]
pub trait AsyncEFromTypeAdapter<'db, 'arena, 'txn, 's>: Sized {
    fn e_from_type(
        self,
        label: &'s str,
...
#[cfg(feature = "slate")]
impl<'db, 'arena, 'txn, 's, S> AsyncEFromTypeAdapter<'db, 'arena, 'txn, 's>
    ...
    > {
        ...
        let stream = async_stream::try_stream! {
            ...
            while let Some(kv) = iter.next().await? {
                ...
                let label_in_db = &value[LMDB_STRING_HEADER_LENGTH..end];
                if label_in_db == label_as_bytes {
                     let edge = Edge::<'arena>::from_bincode_bytes(id, &value, self.arena.get())?;
