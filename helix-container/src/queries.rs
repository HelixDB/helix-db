...
.map_value_or(false, |v| *v == data.name)?)
                } else {
                    Ok(false)
                }
            }).collect_to::<Vec<_>>();
...
.map_value_or(false, |v| *v >= data.date)?)
                } else {
                    Ok(false)
                }
            }).collect_to::<Vec<_>>();
...
.add_n("Patient", Some(props! { "name" => data.patient_name.clone(), "age" => data.patient_age.clone() }), None).collect_to::<Vec<_>>();
