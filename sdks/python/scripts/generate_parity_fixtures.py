...
def with_params(
    ...
    for name, param_type in types:
        request.insert_parameter_type(name, param_type)
    return request
...
def remaining_read_contract() -> Fixture:
    ...
    request.insert_parameter_type("bytes", QueryParamType.bytes())
    request.insert_parameter_type("date_time", QueryParamType.date_time())
...
def main() -> None:
    ...
    if len(runtime_fixtures) != 224:
        raise RuntimeError(
            f"generated {len(runtime_fixtures)} runtime fixtures, expected 224"
        )
